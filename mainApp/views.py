...
def priceFilterPage(Request, mc, sc, br):
    ...
    if mc == "All" and sc == "All" and br == "All":
        ...
        data = Product.objects.filter(
            brand=Brand.objects.get(name=br), finalprice__gte=min, finalprice__lte=max
        ).order_by("-id")
    elif mc != "All" and sc != "All" and br == "All":
        data = Product.objects.filter(
            maincategory=Maincategory.objects.get(name=mc),
...
def searchPage(Request):
    if Request.method == "POST":
        ...
        data = Product.objects.filter(
            ...
            | Q(color__contains=search)
            | Q(size__contains=search)
            | Q(stock__contains=search)
        )
        maincategories = Maincategory.objects.all()
        subcategories = Subcategory.objects.all()
        brands = Brand.objects.all()
...
def singleProductPage(Request, id):
    ...
    relatedProducts = Product.objects.filter(
        maincategory=Maincategory.objects.get(name=data.maincategory)
    )
    return render(
        Request, "detail.html", {"data": data, "relatedProducts": relatedProducts}
    )
...
def contactPage(Request):
    if Request.method == "POST":
        ...
        messages.success(
            Request,
            "Thank You For Sharing Your Query With Us!!! Our Team Will Contact You Soon!!",
        )
...
def profilePage(Request):
    ...
    if user.is_superuser:
        ...
        return render(
            Request,
            "profile.html",
            {"data": buyer, "wishlist": wishlist, "orders": orders},
        )
...
def updateProfilePage(Request):
    ...
    if user.is_superuser:
        ...
        if Request.method == "POST":
            ...
            buyer.adressline1 = Request.POST.get("addressline1")
            buyer.adresslin2 = Request.POST.get("addressline2")
            buyer.adressline3 = Request.POST.get("addressline3")
